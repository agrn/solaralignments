...
let setup_presets longitude_field latitude_field lowest_field highest_field distance_field =
  let write_value value field =
    let num = Js.number_of_float value in
    field##.value := num##toString in
  match Dom_html.getElementById_coerce "preset" Dom_html.CoerceTo.select with
  | None -> ()
  | Some preset_field ->
     preset_field##.onchange :=
       Dom_html.handler (fun _ ->
           ...
           if idx >= 0 then begin
             match List.nth_opt presets idx with
             | None -> ()
             | Some preset ->
...
let get_field id = Dom_html.getElementById_coerce id Dom_html.CoerceTo.input
...
let () =
  ...
  ignore @@
    (get_field "longitude" >>= fun longitude_field ->
     ...
     Some (setup_presets longitude_field latitude_field lowest_field highest_field distance_field))
